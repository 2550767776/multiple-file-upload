...
(function ($) {
    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadSuccess', function (file, response) {
                ...
                // 创建图像
                createBox($fileInput, response.attachemId, response.fileUrl, webUploader);
                if (successCallBack) {
            ...
            // 初始化数据
            if (opt.previewArr) {
                for (let index = 0; index < opt.previewArr.length; index++) {
                    var element = opt.previewArr[index];
                    createBox($fileInput, element.id, element.url, webUploader)
                }
            }
    ...
    //创建文件操作div;
    function createBox($fileInput, id, fileUrl, webUploader) {
        var file_id = id;
        var $parentFileBox = $fileInput.parents(".upload-ul");
